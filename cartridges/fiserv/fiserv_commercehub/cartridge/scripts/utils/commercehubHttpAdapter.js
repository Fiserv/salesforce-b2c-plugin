...
function createSignature(apiKey, payload, timestamp, nonce)
{
...
function populateService(service, payload)
{
    let apiKey = FiservConfig.getCommerceHubApiKey();
    let timestamp = Date.now();
    let nonce = getNonce(timestamp);
    ...
    service.addHeader('Authorization', createSignature(apiKey, payload, timestamp, nonce));
